...
def get_word_score(word, n):
    ...
    word = word.lower()
    for letter in word:
        total_points += SCRABBLE_LETTER_VALUES[letter.lower()]
        wordlen = len(word)
        second_component = max(1, 7 * wordlen - 3 * (n - wordlen))
  
    return second_component * total_points
...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word_counts:
        if hand.get(letter, 0) < word_counts[letter]:
            return False
        return is_valid_word
    
def wildcard_swap(word):
    vowels = "aeiou"
    star_index = word.find("*")
    for vowel in vowels:
        new_word = word[:star_index] + vowel + word[star_index + 1:]
        if new_word in word_list:
            return True

def play_hand(hand, word, word_list):
    total_score = 0
    n = sum(hand.values()) 
...
while sum(hand.values()) > 0:
    ...
    if is_valid_word(word, hand, word_list) == True:
        ...
        total_score += points
        print(f"Earned {points} points. Total score: {total_score}")
